...
Send one `ASSIGNMENT` that names the new slice identifier to the Slice Owner. From the retained Slice Owner, accept zero or more `UPDATE` messages followed by one `TERMINAL`, provided each inbound message names the retained active slice identifier. Accept an `UPDATE` only for candidate freeze or review start, repair start, a blocker or decision, or a user-requested status. For a deferral-decision `UPDATE`, obtain the assigned decision authority's response and relay it unchanged to the retained owner as an `UPDATE`. It must identify the slice, reviewed candidate, exact findings, each `ACCEPT` or `DECLINE`, and a basis for each acceptance. Do not infer a decision or authorize another sink writer.
