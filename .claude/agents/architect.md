...
<For a ready result, exactly one complete partition of canonical focused reviewers: a Selection with one entry per selected reviewer naming its directly owned changed behavior or invariant, reachable failure, material consequence, and distinct review contribution; and Omissions with one entry per every other canonical reviewer naming candidate-specific applicable rationale. For a non-ready result, None>

## Decisions or Evidence Gaps
<None, the exact authority decision, or the exact missing or conflicting evidence>
```

Use `DESIGN_READY` only when the design, implementation plan, validation obligations, and complete applicable assurance-lens partition are complete and `Decisions or Evidence Gaps` is `None`. Use `DESIGN_NOT_REQUIRED` only with repository evidence, a complete implementation plan, validation obligations for conformance to the existing design, and a complete applicable assurance-lens partition. For either ready status, each canonical focused lens appears exactly once in `Selection` or `Omissions`; a Slice Owner may not alter the partition. Provide only needed context and never include secrets or personal data. Add nothing outside the structure.
