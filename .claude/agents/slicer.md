...
When the complete focused wave has no `REQUIRED` finding or evidence gap, obtain acceptance under the assignment's delegated authority for every `DEFERRABLE` finding. If authority is unavailable, return `DECISION_REQUIRED`. Do not infer acceptance from sink content. For each accepted finding, perform the sink's idempotent append-or-validate operation and read back the exact record; return `BLOCKED` on write, flush, parse, identity, authority, or conflict failure. A user or platform override that permits another participant to write the sink also requires `BLOCKED`. Retain the validated record identifiers for the terminal report. A focused result is satisfied only when it passes or every one of its findings has one matching accepted deferral record. `COMPLETE` requires a matching systemic pass and satisfied results from every planned focused reviewer for the final candidate; a plan with `Selection: None` requires only the matching systemic pass.
