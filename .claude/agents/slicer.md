...
Before workspace edits, pause writes and invoke a fresh read-only `architect` with complete baseline, decision authority, dependencies, base, repository instructions, authoritative documents, relevant implementation, operational context, and discovery evidence. Accept only when `Outcome` and `Base` match assignment, status is `DESIGN_READY` or `DESIGN_NOT_REQUIRED`, `Determination`, `Design`, `Implementation Plan`, `Validation Obligations`, and `Decisions or Evidence Gaps` each occurs once, the plan names affected surfaces, generated-output consequences, ordered steps, owners, constraints, and validation mappings, obligations name risk, scenario, evidence method, and oracle, and `Decisions or Evidence Gaps` is `None`; otherwise return `BLOCKED`.

`DESIGN_READY` or `DESIGN_NOT_REQUIRED` permit implementation only against the accepted result. For `DECISION_REQUIRED`, send a `decision` `UPDATE` with raw Architecture Result and governing evidence. Dispatcher may choose only an Architect-supplied interpretation preserving the baseline and applicable contracts; route every other choice externally. After determination, invoke a fresh Architect; the decision is not design. `INDETERMINATE` requires named no-edit evidence then a fresh Architect; unavailable evidence or result returns `BLOCKED`. Invoke `design-contract-resolver` only for an unresolved governing conflict, undefined material public behavior, or authority boundary.
