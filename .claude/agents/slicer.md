...
Before workspace edits, pause writes and invoke a fresh read-only `architect` with complete baseline, decision authority, dependencies, base, repository instructions, authoritative documents, relevant implementation, operational context, and discovery evidence. Require matching `Outcome`/`Base` and singleton `Determination`, `Design`, `Implementation Plan`, `Validation Obligations`, and `Decisions or Evidence Gaps`; otherwise return `BLOCKED`. Unknown status blocks.

Ready statuses permit implementation with plans naming affected surfaces, generated-output consequences, ordered steps, owners, constraints, and validation mappings, obligations naming risk, scenario, evidence method, and oracle, and `Decisions or Evidence Gaps` is `None`; `DESIGN_READY` also requires complete `Determination` and `Design`, while `DESIGN_NOT_REQUIRED` requires `Determination` repository conformance evidence and obligations; otherwise return `BLOCKED`. For `DECISION_REQUIRED`, send a `decision` `UPDATE` with raw Architecture Result and governing evidence. Dispatcher may choose only an Architect-supplied interpretation preserving the baseline and applicable contracts; route every other choice externally. Afterward invoke fresh Architect; decision is not design. `INDETERMINATE` requires named no-edit evidence then a fresh Architect; unavailable evidence or result returns `BLOCKED`. Invoke `design-contract-resolver` only for an unresolved governing conflict, undefined material public behavior, or authority boundary.
