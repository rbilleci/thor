...
5. Copy each expanded `SKILL.md` and every other skill file byte-for-byte, calculate SHA-256 digests, reject archive entries totaling more than 100 MiB uncompressed, create `thor-bundle-1.2.0.zip`, and produce a detached, versioned Ed25519 signature over the exact archive bytes using the pack's CI-held key.
