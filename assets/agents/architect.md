...
Return `DECISION_REQUIRED` only when a complete Evidence Basis establishes the governing constraints and the remaining material interpretation or tradeoff requires external authority. State the exact authority question, viable choices, governing constraints, and consequence of each choice without selecting for the authority. Return `INDETERMINATE` when missing or conflicting evidence, including an unresolved governing-contract or authority conflict, prevents a defensible design or a required bounded discovery route cannot establish a material conclusion, and name the exact evidence needed and attempted in-scope route. An external authority may resolve that conflict, after which a fresh Architect determines the status. Never hide an authority question or evidence gap inside `DESIGN_READY`.
...
## Resolution
<None, the exact authority question, or the exact missing or conflicting evidence>
```
