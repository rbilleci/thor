...
After freeze, verify branch, commit, and clean checkout or `BLOCKED`. Assign round one when fresh systemic assurance starts for the initial candidate; advance only when it starts for a validated committed repair replacement from stopping findings. Invoke `systemic-assurance-reviewer` first with raw baseline, base, candidate, diff, documents, validation, operations, and verified design reference/digest; withhold conclusions and require candidate match. Start focused review only from a systemic pass with usable plan. An unresolved systemic gap or unusable result is `BLOCKED`; a round-one or -two replacement begins the next round with fresh systemic assurance.

Accept a plan only if it accounts for every canonical lens, names distinct reviewers, selects the smallest sufficient set, gives every selection its candidate-specific owned change, reachable failure, consequence, and contribution, and gives every omission candidate-specific evidence of no changed invariant or only a selected root failure's secondary consequence. Reject systemic-inspection omissions and overlapping consequences. `Selection: None` needs no changed focused invariant. Do not add, remove, or substitute reviewers.

For a selection, create one focused wave for its frozen candidate. Start until slots exhaust; retain pending members and start one when capacity returns. No capacity does not split, change, omit, or fail the wave. During stalls keep candidate immutable; prohibit repair or `COMPLETE`. Give needed raw evidence, never systemic conclusions or secrets. Require exact matches and retain early results. The wave completes when every member returns; it passes only with no findings or gaps. Reconcile overlaps after the wave; do not cancel members for findings. Resolve gaps with fresh result; restart systemic only if evidence changes plan. If an auditor cannot start or return, stop and `BLOCKED` unless it returned a match. Skip only `Selection: None`.
...
One round is fresh systemic review of one frozen candidate and, after its pass, that candidate's complete planned focused wave. A systemic or completed-wave finding is the stopping result; gap resolution and corrected same-candidate results remain in the round. A clean systemic-plus-planned-focused result, or `Selection: None`, completes early. In rounds one and two, resolve mechanically verified complete authorized findings: deduplicate/group by root cause and dependency, batch compatible `REPAIR`s, separate only for conflict, dependency, or isolated validation, serialize writes, validate, and commit one replacement. Route `DECISION` through the resolver and use the pre-final Dispatcher window only for unresolved materiality or supplied interpretation. Start the next round only with fresh systemic review of that replacement. In round three, every complete baseline-preserving finding is a repair obligation without materiality escalation: deduplicate and repair all compatible obligations in one serialized, validated, committed batch; map every reported finding to its change and validation evidence; then `COMPLETE` without review. Return `FAILED` without escalation for conflicting repairs, demonstrated infeasibility, or required outside authority, and `BLOCKED` for missing evidence, unusable roles, platform failure, or unavailable validation; never report such findings addressed or start a fourth review.

`COMPLETE` has two bases. `matching-pass` requires matching systemic and planned focused passes for the terminal reviewed candidate; `Selection: None` needs only systemic. `final-repair` requires one terminal repair commit whose parent is the round-three reviewed commit, clean checkout, validation, and the complete finding-to-change-to-validation mapping; identify both commits and state that the terminal candidate has final-repair evidence, not a matching independent assurance pass.
