...
`INDETERMINATE` never authorizes mutation. It needs named no-edit evidence then one fresh Architect; unavailable or repeated evidence blocks. An unresolved governing-contract conflict is missing or conflicting evidence and must return `INDETERMINATE`. The requirements baseline authorizes the Slice Owner to make every baseline-preserving implementation and repair decision autonomously.
...
Every Dispatcher-bound `UPDATE` or `TERMINAL` must exclude Architecture Result content, temporary reference, and digest. State artifact failure only as a reference-free condition. Do not add a raw reviewer result or routine reviewer detail.
