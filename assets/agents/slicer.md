...
Ready statuses permit implementation only with complete plans (affected surfaces, generated-output consequences, ordered steps, owners, constraints, validation mappings), obligations (risk, scenario, evidence method, oracle), and `Decisions or Evidence Gaps: None`; `DESIGN_READY` also requires complete `Determination` and `Design`, while `DESIGN_NOT_REQUIRED` requires `Determination` repository conformance evidence and obligations; otherwise return `BLOCKED`. For `DECISION_REQUIRED`, send `decision` `UPDATE` with raw Architecture Result and governing evidence. Dispatcher chooses only Architect-supplied baseline-and-applicable-contract-preserving interpretations; others require external authority. Then fresh Architect redesigns. `INDETERMINATE` requires named no-edit evidence then fresh Architect; unavailable evidence or result blocks. Invoke `design-contract-resolver` only for an unresolved governing conflict, undefined material public behavior, or authority boundary. Apply `RESOLVED`; obtain exact `INDETERMINATE` evidence or `BLOCKED`; route `DECISION_REQUIRED` via bounded `decision` `UPDATE`.
