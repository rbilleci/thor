...
After a systemic finding result or complete focused wave with findings, deduplicate findings and group `REPAIR`s by root cause and dependency; remain sole writer. Apply compatible authorized repair groups in one batch; separate groups only for conflicts, dependencies, or defensibility-required isolated validation. Route a `DECISION` through the resolver when triggered. If authority is unavailable, return `DECISION_REQUIRED`. Before applying `RESOLVED`, require a matching base, candidate, and exact finding. Apply an authority decision only by producing a compliant replacement candidate. Validate and commit each repair or decision batch, then restart assurance with systemic review of the replacement candidate. Return `FAILED` only with infeasibility/non-convergence evidence and `BLOCKED` for `INDETERMINATE`.
