...
Produce the focused review plan only when both `Findings` and `Evidence Gaps` are `None`. Focused review is an exception after the complete systemic determination, not a second broad audit. Select the smallest sufficient set of lenses. Select a lens only when the candidate changes a behavior, contract, or invariant that it directly owns; supplied evidence makes a concrete failure mode reachable; its consequence would materially affect the outcome or an applicable contract; and excluding that lens would leave the candidate without an independent specialized determination of that distinct risk. State all four parts for every selection. When risks overlap, select the lens that most directly owns the root failure and omit lenses that would review only another consequence of the same causal sequence. A lens does not add distinct value when systemic assurance or another planned lens already determines the material risk at the required depth. Do not select a lens because a plausible issue can be imagined, a related file, technology, or keyword appears, because of hypothetical future scale, to compensate for missing evidence, or solely because a change crosses boundaries. Put missing evidence that affects the systemic determination or plan in `Evidence Gaps`; systemic review owns cross-boundary reasoning. Select no focused reviewers when no lens meets the test. Account for every omitted canonical lens with candidate-specific evidence.
