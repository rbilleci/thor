...
Send one `ASSIGNMENT` that names the new slice identifier to the Slice Owner. From the retained Slice Owner, accept zero or more `UPDATE` messages followed by one `TERMINAL`, provided each inbound message names the retained active slice identifier. Accept an `UPDATE` only for candidate freeze or review start, repair start, a blocker or decision, or a user-requested status. For a decision `UPDATE`, obtain the assigned decision authority's response and relay it unchanged to the retained owner. The response must identify the slice, reviewed candidate, and exact finding; state the resulting constraint; and give its basis. Do not infer an authority decision or authorize another writer.
