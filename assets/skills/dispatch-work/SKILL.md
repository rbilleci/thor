...
Define an `ASSIGNMENT` containing the requirements baseline—an independently verifiable outcome, scope, non-goals, constraints, and acceptance criteria—plus decision authority, dependencies, base commit, assigned branch, and checkout. Assign the Work Dispatcher authority to determine an escalated finding's materiality and choose among resolver-supplied interpretations only when the decision preserves that baseline and every applicable contract. Reserve changes to the baseline or an applicable contract for external authority. Use a new slice identifier. Before sending the `ASSIGNMENT`, verify that the current branch equals the assigned branch, `HEAD` equals the base commit, and `git status --porcelain` is empty. If a check fails, return `BLOCKED` to the user without altering the checkout. Launch one `slicer` subagent as the Slice Owner and make it the checkout’s only writer. Do not create another Git worktree or branch.

Send one `ASSIGNMENT` that names the new slice identifier to the Slice Owner. From the retained Slice Owner, accept zero or more `UPDATE` messages followed by one `TERMINAL`, provided each inbound message names the retained active slice identifier. Accept an `UPDATE` only for candidate freeze or review start, repair start, a blocker or decision, or a user-requested status. For a decision `UPDATE` within its assigned authority, decide only from the supplied baseline, raw finding or resolver result, and governing evidence, then relay the exact determination to the retained owner. The determination may decide whether complete proof satisfies the finding obligation or select one resolver-supplied interpretation that preserves the baseline and every applicable contract. It may not change the baseline or an applicable contract, accept risk, declare `COMPLETE`, direct implementation, authorize another writer, or extend the repair-round limit. For a decision outside this authority, obtain the external authority's response and relay it unchanged.
